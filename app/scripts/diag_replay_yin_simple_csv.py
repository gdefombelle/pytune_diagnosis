...
def consolidate_chosen_note(
    chosen: Optional[dict],
    chosen_method: Optional[str],
    candidate_top_notes_list: list[str],
    hps_info: dict,
    fft_band_selected: Optional[str],
    band_rows: list[dict],
    hps_score_threshold: float = 0.75,
) -> tuple[Optional[dict], Optional[str]]:
    """
    Consolidation prudente :
    - on part du chosen YIN/FFT existant
    - on ne remplace que si HPS est dans les candidates YIN
    - hps_score doit être suffisamment élevé
    - et il faut confirmation par fft_band_note ou fft_band_selected
    """
    chosen2 = chosen.copy() if chosen else None
    chosen_method_2 = chosen_method

    if not candidate_top_notes_list:
    ...
    candidate_hps_note = None
    if hps_note is not None and hps_note in candidate_top_notes_list:
        candidate_hps_note = hps_note
    elif hps_multi_note is not None and hps_multi_note in candidate_top_notes_list:
        candidate_hps_note = hps_multi_note

    if candidate_hps_note is None:
        return chosen2, chosen_method_2

    fft_notes = {
        str(r.get("fft_band_note"))
        for r in band_rows
        if r.get("fft_band_note") is not None
    }

    fft_confirms = (
        candidate_hps_note == fft_band_selected
        or candidate_hps_note in fft_notes
    )
    ...
    new_f0 = None
    if hps_note == candidate_hps_note and hps_info.get("hps_f0") is not None:
        new_f0 = hps_info.get("hps_f0")
    elif chosen2 and chosen2.get("refined_note") == candidate_hps_note:
        new_f0 = chosen2.get("refined_f0")
    else:
        for r in band_rows:
            if r.get("candidate_note") == candidate_hps_note:
                new_f0 = r.get("candidate_f0")
                break

    chosen2 = {
        "refined_note": candidate_hps_note,
        "refined_f0": new_f0,
        "refined_score": float(hps_score),
    }
    chosen_method_2 = "consolidated_hps_fft"
...
def main():
    parser = argparse.ArgumentParser(
        description="CSV YIN scan + non-informed fft_band + HPS global + conservative consolidation."
    )
    ...
    parser.add_argument("--hps-score-threshold", type=float, default=0.75)
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        chosen2, chosen_method_2 = consolidate_chosen_note(
            chosen=chosen,
            chosen_method=chosen_method,
            candidate_top_notes_list=candidate_top_notes_list,
            hps_info=hps_info,
            fft_band_selected=fft_band_selected,
            band_rows=band_rows,
            hps_score_threshold=args.hps_score_threshold,
        )
