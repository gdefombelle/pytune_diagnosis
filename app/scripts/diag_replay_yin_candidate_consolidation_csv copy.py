...
def compute_hps_local_candidate(
    ...
    try:
        local_fft_f0, _ = fft_band_f0(signal, sr, fmin, fmax)
        if local_fft_f0 is not None:
            hps_f0 = float(local_fft_f0)
            hps_note = note_name_or_none(hps_f0)
    except Exception:
        pass
...
def build_candidate_consolidation(
    ...
    for note, base in best_per_note.items():
        ...
        # Bonus spécial harmonique basse pour aider C2/C#2/etc.
        if refined_row is not None:
            try:
                refined_note = refined_row.get("refined_note")
                if refined_note == note and fft_local_note == note:
                    final_score += 0.45
...
def choose_octave_corrected_candidate(
    ...
    Correction d'octave spécialisée :

    - ne regarde QUE les variantes d'octave de chosen_2

    - s'appuie sur FFT locale + refined_note

    - n'utilise PAS candidate_hps_local_note pour éviter les régressions graves
    ...
    for note, cand in candidate_map.items():
        ...
        # bonus octave volontairement fort

        alt_score_aug = alt_score + 0.60 * local_support

        # si la note actuelle n'a pas ce support local, on favorise le saut

        current_local_support = 0.0

        if current.get("candidate_fft_local_note") == chosen_note_2:

            current_local_support += 1.0

        if current.get("refined_row") is not None:

            try:

                if current["refined_row"].get("refined_note") == chosen_note_2:
...
def choose_cross_note_rescue_candidate(
    ...
    Rescue cross-note prudent :

    - part de chosen_3

    - ne touche JAMAIS aux variantes d'octave de chosen_3

    - autorise une autre note seulement si HPS global la soutient
    ...
    for note, cand in candidate_map.items():
        ...
        # IMPORTANT : chosen_4 ne doit pas annuler une correction d'octave

        if is_same_pitch_class_octave_variant(chosen_note_3, note):
...
def main():
    parser = argparse.ArgumentParser(
        description="YIN candidate consolidation with FFT local, HPS local, HPS band, octave correction, and cross-note rescue."
    )
    ...
    parser.add_argument("--octave-min-gain", type=float, default=0.15)
